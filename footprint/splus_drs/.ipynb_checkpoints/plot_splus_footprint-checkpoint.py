...
ebv_cmap = truncate_colormap(cm.turku_r, 0, 0.5)
#ebv_cmap = truncate_colormap(cm.grayC, 0, 0.5)
...
for ra in ra_list:
    for dec in dec_list:
        ebv_map["ra"].append(ra)
        ebv_map["dec"].append(dec)

m = sfdmap.SFDMap(ebv_maps_path)
ebv_map["ebv"] = m.ebv(ebv_map["ra"], ebv_map["dec"])

c = SkyCoord(ra=ebv_map["ra"]*u.degree, dec=ebv_map["dec"]*u.degree)
ebv_map["l"] = c.galactic.l.degree
ebv_map["b"] = c.galactic.b.degree

ebv_map = pd.DataFrame(ebv_map)

f = ebv_map["l"] > 180
ebv_map.loc[f, "l"] = ebv_map.loc[f, "l"] - 360
...
if not galactic:
    cmap = plt.scatter(-ebv_map["ra"]*np.pi/180, ebv_map["dec"]*np.pi/180, c=np.log10(ebv_map["ebv"]), cmap = ebv_cmap, marker="s", s=3, alpha=1, vmin = -2.5, vmax = 0.5)
    plt.xlabel('RA')
    plt.ylabel('DEC')

else:
    cmap = plt.scatter(-ebv_map["l"]*np.pi/180, ebv_map["b"]*np.pi/180, c=np.log10(ebv_map["ebv"]), cmap = ebv_cmap, marker="s", s=3, alpha=1, vmin = -2.5, vmax = 0.5)
    plt.xlabel('l')
    plt.ylabel('b')
